...
void studentas::readStudentai(studentas &temp, vector <studentas> &mas, double &skaitym, string failodydis) {
  skaitymas(temp, mas, skaitym, failodydis);
}
...
void isvedimas2 (vector <studentas> &temp, string failodydis, string tipas, double &isved) {
    ...
    for(auto &i: temp) {
        sprintf(eil, "%-15s%-15s", i.getVardas().c_str(), i.getPavarde().c_str());
        laikeilute=eil;
